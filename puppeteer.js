    ...
    // 나머지 코드...
		for (const url of pageUrls) {
			...
			try {
				...
				// const isSnum = await page.$(name_ko);
				// console.log(isElementExist, isSnum);
				// return;

				if (isElementExist) {
					// 선택자에 일치하는 내용을 추출
					const customText = await page.$eval(name_en, element => element.textContent);
					// const elementContentName_en = await page.$eval(name_en, element => element.textContent);
					const elementContentName_en = customText.replace(/\s/g, '').replace(/\//g, ',');
					
					// const elementContentName_ko = await page.$eval(name_ko[0], element => element.textContent);
					...
					const reMsg = `No element found for name_en "${name_en}" on ${url}`;
					console.error(reMsg);
					resultData += reMsg+'\n';
				}
				// 데이터를 CSV 문자열로 변환
				// csvData = data.map(item => `${item.url},${item.nameEn},${item.nameKo}`).join('\n');
				csvData = data.map(item => `${item.url},${item.nameEn}`).join('\n');
			} catch (error) {
		...
		fs.writeFileSync('output.csv', 'url,p_num,name_en\n' + csvData, 'utf-8');
